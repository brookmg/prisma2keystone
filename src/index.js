...
const options = yargs   
    ...
    .option("o", { alias: "output", describe: "Keystone file path", type: "string", demandOption: false, default: 'generated.ts' })
    .option("enum" , { alias: "generate-enums" , describe: "Generate Enums inside the prisma schema", type: "flag", demandOption: false })
    .argv;
    ...
    return (
        ts.factory.createPropertyAssignment(
            ...
            ts.factory.createCallExpression(
                ...
                undefined, innerObject && innerObject.length > 0 ? [
...
async function writeTSFile(parsed) {
    ...
    parsed.models.forEach(item => modelNameToFieldsMap.set(item.name , item.fields))
    ...
    if (options.enum) {
        for (let e of enumNameToValues.keys()) {
            ...
            en.addMembers(enumNameToValues.get(e).map(item => {return { name: item}}));
        }
